...
const validate = values => {
    ...
    if(values.pass!==values.pass2){
        errors.pass2 = 'Password must repeat perfectly'
    }
    return errors
}
...
class SwiperForm extends Component {
    render() {
                ...
                { (!user.isHasUser)?
                    <form onSubmit={handleSubmit}>
                        ...
                        <Field className="form-control form-control-has-validation form-control-last-child"
                               label="Email"
                               type="email" validate={[required]} name="email" component={renderField}/>
                        <Field className="form-control form-control-has-validation form-control-last-child"
                               label="Password"
                               type="password" validate={[required]} name="pass" component={renderField}/>
                        <Field className="form-control form-control-has-validation form-control-last-child"
                               label="Repeat your password" validate={[required]} type="password" name="pass2"
...
SwiperForm = connect(mapStateToProps)(SwiperForm);
